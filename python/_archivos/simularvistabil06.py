...
def aplicar_configuracion_estetica():
    ...
    st.markdown("""
        ...
        """, unsafe_allow_html=True)
# ==========================================
# BLOQUE 2: EL APLICATIVO (CAJA NEGRA) - MODIFICADO (MODO AUTO-ENTRENA)
# ==========================================
def ejecutar_aplicativo_principal():
    """Aquí reside todo el código que ya funciona impecablemente con el nuevo modo Auto-Entrena"""
    ...
    # --- Lógica de Interfaz Principal e Inicialización ---
    if 'data' not in st.session_state:
        st.session_state.update({
            ...
            'auto_entrena': False # Nueva variable para el modo manos libres
        })
    ...
    if st.session_state.data or st.session_state.modo_falla_activo:
        ...
        if st.session_state.viendo_resumen:
            # (Mantiene la lógica de resumen intacta...)
            ...
            if not st.session_state.data:
                st.warning("⚠️ No hay datos cargados. Por favor, selecciona una categoría en el panel lateral y presiona 'Cargar'.")
            else:
            ...
            # --- MODIFICACIÓN: MODO ENTRENA CON OPCIÓN AUTO ---
            if not st.session_state.auto_entrena:
                # MODO MANUAL (El original)
                ...
                with c_de:
                    ...
                    st.markdown("---")
                    if st.button("🚀 MODO AUTO"): st.session_state.auto_entrena = True; st.rerun()
            else:
                # MODO AUTOMÁTICO (MANOS LIBRES)
                st.info("Modo Automático Activo: Relájate y escucha.")
                placeholder = st.empty()
                if st.button("🛑 DETENER"): st.session_state.auto_entrena = False; st.rerun()

                # El bucle recorre desde el índice actual hasta el final
                for i in range(st.session_state.idx_entrena, len(current_subset), 2):
                    ...
                    # FASE 2: Mostrar Traducción (Inglés)
                    with placeholder.container():
                        st.markdown(f"<div class='stat-box'>{progreso} / {total_int}</div>", unsafe_allow_html=True)
                        st.markdown(f'<div class="word-box"><h3>ESPAÑOL:</h3><h1>{v_p}</h1></div>', unsafe_allow_html=True)
                    ...
                    time.sleep(3) # Tiempo para que termine el audio y asimile
            ...
            # (Se mantiene igual tu lógica de juego con el TOAST para Edwin...)
            mostrar_identidad_usuario()
                    ...
                    with c_j2:
                        ...
                        if st.session_state.j_paso == 1:
                            ...
                            if st.button("COMPROBAR"):
                                ...
                                opciones_validas = [o.strip().lower() for o in v_s.split('/')]
                                
                                if ans in opciones_validas:
                                    st.session_state.j_status = "OK"
                                    st.session_state.aciertos += 1
                                    st.toast(f"¡Sigue así Edwin, vas muy bien! 🔥 ({st.session_state.aciertos} aciertos)", icon='✅')
        ...
        # Cierre del bloque de validación si hay data cargada
    else:
        st.warning("No hay datos cargados. Por favor, selecciona los parámetros en el panel lateral y pulsa 'Cargar Nueva Data'.")

# Finalmente, fuera de la función principal, si el usuario está autenticado, se ejecuta la lógica
