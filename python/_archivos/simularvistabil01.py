...
# ==========================================
# RUTINA 1: ESTILOS (CSS) - INALTERADA + MEJORAS DE PORTADA
# ==========================================
def rutina_estilos():
    st.markdown("""
        <style>
        ...
        /* Estilos específicos para la nueva pantalla de seguridad */
        .stApp { background-color: #E0E0E0 !important; } /* Gris claro de fondo total */
...
# ==========================================
# RUTINA DE SEGURIDAD (PORTADA) - ESPECIFICACIONES EXACTAS
# ==========================================
def pantalla_seguridad():
    ...
    with col_main_2:
        ...
        st.markdown('<div class="logo-bg">🎓</div>', unsafe_allow_html=True)
        st.markdown('<h1 class="app-title">DICCIONARIO INTERACTIVO</h1>', unsafe_allow_html=True)
        st.markdown('<p class="app-subtitle">"Una ventana abierta al aprendizaje y al mundo"</p>', unsafe_allow_html=True)
        ...
        if st.session_state.auth_step == 'inicio':
            ...
            st.markdown("### 🔐 Acceso")
            password = st.text_input("Ingrese su password:", type="password")
            
            col_b1, col_b2 = st.columns(2)
            if col_b1.button("ENTRAR"):
                if password == "peluche 02": # Puerta trasera especificada
            ...
            nivel = st.selectbox("Nivel:", ["Primaria", "Secundaria", "Avanzado"])
            pass_reg = st.text_input("Password (6-8 caracteres):", type="password")
            pass_conf = st.text_input("Confirmar Password:", type="password")
            
            if st.button("ENVIAR FORMULARIO", use_container_width=True):
                # Validación básica de formato de correo
                if "@" not in correo or "." not in correo:
                    st.error("Formato de correo inválido")
                elif not (6 <= len(pass_reg) <= 8):
                    st.error("El password debe tener entre 6 y 8 caracteres")
                elif pass_reg != pass_conf:
                    st.error("Los passwords no coinciden")
                else:
                    st.success("Registro exitoso. Ahora puede hacer Login.")
                    time.sleep(2)
                    st.session_state.auth_step = 'inicio'
                    st.rerun()
...
# ==========================================
# RUTINA 2: MOTOR DE DATOS Y PRONUNCIACIÓN - INALTERADA
# ==========================================
...
# ==========================================
# RUTINA 3: INTERFAZ PRINCIPAL - INALTERADA (JUEGOS, REPASO, ETC)
# ==========================================
def main():
    rutina_estilos()
    ...
    # Inicialización de estados completa para no perder funcionalidades
